...
class SkipList:
    ...
    def _createHistogram(self) -> List:
        ...
        for i in range(1, self.max_level+1):
            H[i] = ceil(self.n * (1 - pow(self.p, i)))
        return H
    ...
    def insert(self, value: int) -> None:
        ...
        while curr_level < level:
            ...
            pred_level = len(pred_node.pointers) - 1
            while pred_node is not self.head and pred_level < curr_level:
                pred_node = pred_node.predecessor

            head_level = len(self.head.pointers) - 1
            if pred_node is self.head and head_level < curr_level:
                self.head.pointers.append(None)

            new_node.pointers[curr_level] = pred_node.pointers[curr_level]
            pred_node.pointers[curr_level] = new_node
